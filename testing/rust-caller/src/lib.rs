...
use ocaml_interop::{OCaml, OCamlBytes, OCamlRuntime, IntoOCaml, ToOCaml};
#[cfg(test)]
...
pub fn allocate_alot(cr: &mut OCamlRuntime) -> bool {
    ...
    for _n in 1..50000 {
        let _x: OCaml<OCamlBytes> = vec.as_slice().into_ocaml(cr);
        let _y: OCaml<OCamlBytes> = vec.as_slice().into_ocaml(cr);
        let _z: OCaml<OCamlBytes> = vec.as_slice().into_ocaml(cr);
    }
    true
}
